...
class Reportchecklist extends MY_Controller {
	...
	public function exportpdf(){
        ...
        $this->hospital_model->id = $hospital_id;
        $hospital = $this->hospital_model->getItemById();
		...
		foreach ($days as $items){
            ...
            $dayOfJapana = $this->department_model->dayOfJapanese( date('N', strtotime($items->date_add)) );
			$items->fdate_add = date_format($date, 'j')." ($dayOfJapana)"; // day of month
			//date("w", strtotime($item->date_add));
		}
        ...
        $this->_data['list'] = $dataTotalCheckListSubmit;
		$this->_data['sum_percent'] = round($sum_percent/$total_day);
        $monthJapanese =  '月 分'; //$this->department_model->monthJapanese();
		$datereport = date($yearValue.' 年 '.(int)$monthValue.' '.$monthJapanese);
        ...
        $this->_data['hospital_name'] = $hospital->hospital_name;
	...
	public  function writerpdf( $sum_percent, $report_comment, $staff_report, $str ){
        ...
        $pdf = new TCPDF("P", "mm", "A3", true, "UTF-8", false); // Bang update
        // $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		
        $title_page = mb_convert_encoding( $this->lable['title_page'], 'sjis','utf-8');
		// set document information
		$pdf->SetCreator(PDF_CREATOR);
		//$pdf->SetAuthor('Nicola Asuni');
		$pdf->SetTitle( $title_page );
		//$pdf->SetSubject('TCPDF Tutorial');
		//$pdf->SetKeywords('TCPDF, PDF, example, test, guide');

		// set default header data
		$PDF_HEADER_TITLE = $title_page;
		$PDF_HEADER_STRING = '';
		...
		// add a page
		$pdf->AddPage();		
	$footer_image_file = FCPATH . 'assets/admin/darkgreen/img/logo_white.png';

$htmlFooter = <<<EOF
<style>
    html, body {
    font-family: Arial,sans-serif;
    font-weight: 500;word-break: normal;-webkit-font-smoothing: antialiased;-moz-osx-font-smoothing: grayscale;-webkit-text-size-adjust: 100%;
	}
	.text-c{
		text-align: center;
	}
...
EOF;
		...
		<table style="width:100%;" class="wcorlor">
			<tr>
				<td class="w30 text-c" style="background-color:#A72529;height:35px;">
				    <br />
					<div>
				    &nbsp; <img style="height:50px;" src="'. $footer_image_file .'" alt="A-Line" />
				    </div>
				</td>
				<td class="w60 text-r" style="background-color:#A72529;height:35px;">
					<br />
					<div>
                        〒104-0061 東京都中央区銀座4-5-1<br/>
		...
		<div class="text-c wcorlor h25" style="background-color:#A72529;font-weight:bold;font-size:18px;">
			Copyright <img style="height:15px;" src="' . $this->_data['base_tlp_admin'] . '/img/Copyright_white.svg.png" alt="@"> A-LINE. All rights reserved.
		...
		$pdf->writeHTMLCell(0, 0, $PDF_MARGIN_LEFT, 370, $htmlFooter, 0, 0, false, true, 'P', false);
		...
		$pdf->Output( $filename, 'D', true); // I output
