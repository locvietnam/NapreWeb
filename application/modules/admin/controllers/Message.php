...
class Message extends MY_Controller{
	...
	public function index(){
			...
			$where = " Where a.user_id != " . $user_id . ' AND a.avail = 1 ';
			if( $user->role_id == 5 ) {
			...
			if($this->input->post()) { 
				...
				if( $data['user_id'] != '' ) {
					...
					$dataInsert['message_type'] = ($user->role_id <= 3 ) ? 'A' : 'U';//change HR to A
					$dataInsert['contents'] = $data['message'];				
					$dataInsert['date_add'] = date('Y-m-d H:i:s');
					$dataInsert['last_update'] = date('Y-m-d H:i:s');			
					$id_insert = (int)$this->message_model->insertItem( $dataInsert );
					...
					if( $id_insert > 0 ){
						...
						$issend = 2;
						///redirect(admin_url('messages/failure'));
				...
				redirect(admin_url('message.html?issend=' . $issend));
			}
			
			switch( (int)$this->input->get('issend') ){
				case 1:
					$this->_data['send_message'] = $this->lable['send_message_success'];
					$this->_data['alert_class'] = 'alert-success';
				break;
				case 2:
					$this->_data['send_message'] = $this->lable['send_message_failure'];
					$this->_data['alert_class'] = 'alert-error';
				break;
				case 3:
					$this->_data['send_message'] = $this->lable['not_choose_receiver'];
					$this->_data['alert_class'] = 'alert-warning';
				break;
			}
